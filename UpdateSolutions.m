...
function [f,p,GTeacher, GTeacherValue,Xmean,PenaltyVoltage,PenaltyVoltageBest] = ...
    UpdateSolutions(GTeacher, p,Xmean,f,PenaltyVoltage,LoadOffPeak,LoadMedium,LoadPeak)
...
for i = 1:size(p,1)
 %%%%%%%%%%%%%%% Teacher phase %%%%%%%%%%%%%%%       
    TF = round(1+rand);   
    pnew(i,:) = p(i,:) +rand(1,size(p,2)).*(GTeacher - TF*Xmean);
    pnew(i,:) = round(pnew(i,:));
  %%%%%% 
...
PenaltyVoltageNew(i) = sum(PenaltyVoltageL(i,:),2);
   fnew(i) = Ke*(T_OffPeak*PLoss(i,1) + T_Medium*PLoss(i,2) + ...
       T_Peak*PLoss(i,3)) + Kp*PLoss(i,1) + Total_Cap_Price;    

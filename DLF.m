...
% Strdata=xlsread('DLF.xls','Strdata');
% Loaddata=xlsread('DLF.xls','Loaddata');
...
Sbus(Loaddata(:,1))=(Loaddata(:,2)+i*Loaddata(:,3))*1000;
Cbus(1,:)=[];
Sbus(1,:)=[];
...
%S_bus=Sbus.*(Vbus/Vbase)-i*(Cbus.*(Vbus/Vbase).^2);  %for I constant
